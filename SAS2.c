...
void initProduits()
{
    ...
    listP[1].prix = 499;
    strcpy(listP[1].descriptions, "smartphone noir andruide");
    ...
    listP[3].prix = 500;
    strcpy(listP[3].categorie, "fourniture");
...
void rechercherProduit_categorie(){
    ...
    for(i=0; i<4; i++){
    if(!strcmp(categorieRech,listP[i].categorie)) 
   { printf("ID :%d\n",listP[i].idProduit);
    printf("stock: %d\n",listP[i].stock);
    printf("%s\n",listP[i].nom);
    printf("%f\n",listP[i].prix);
...
void stock_actuel(){
   int nvStock=0;
   nvStock =listP[id-1].stock-qu;
   printf("Stock actuel%d",nvStock);
}
